...
export const updateSubject = (item) => {
  return (dispatch) => {
    ...
    return axios
      .post(`${process.env.REACT_APP_API_URL}/subjects/${item.id}`,item)
      .then(({data}) => {
        dispatch({ type: SET_SUBJECTS_IS_LOADING, payload: false });
        dispatch({ type: SET_SUBJECTS_UPDATE_IS_UPDATE, payload: true });
        dispatch({ type: SET_SUBJECTS_UPDATE_ITEM, payload: data });
        dispatch({ type: SET_SUBJECTS_MESSAGE, payload: "Subject is update" });
      })
      .catch((err) => {
        ...
        dispatch({ type: SET_SUBJECTS_MESSAGE, payload: "Subject not update" });
      });
  };
};
